...
条件：
- **過去に推薦した曲**（{history_text}）と**同じ曲は絶対に選ばないで**ください。
- **超有名な曲**（例：千本桜、メルト、ボカロPの代表作など）は**絶対に選ばないで**ください。
- **知名度が低すぎる曲や、再生回数が極端に少ない曲は選ばないで**ください。一般的なVocaloidファンなら知っている程度の曲を選んでください。
- 形式は必ず『推薦曲名：<曲名>』のみとし、その他の説明、顔文字、余計な言葉は**禁止**です。
